...
# =============================================================================
Sample = namedtuple('Sample', ['tdc_deg', 'di', 'pressure'])
...
class Angus:
    ...
    def decode(words):
        """
        Decode one 2-word DMA sample.

        words: sequence of 2 unsigned 32-bit integers.
        ...
        w = [int(words[i]) & 0xFFFF_FFFF for i in range(2)]
        return Sample(
            tdc_deg  = w[0] * _TDC_LSB,
            di       = (w[1] >> 24) & 0xFF,
            pressure = (w[1] >>  4) & 0x0FFF,
        )
    ...
    def decode_buffer(buf, n_samples=None):
        ...
        buf:       flat array/list of 32-bit words (2 words per sample).
        ...
        return [Angus.decode(buf[i * 2 : i * 2 + 2]) for i in range(n)]
